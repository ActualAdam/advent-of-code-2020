...
val puzzleInput = InputReader.lines(11, "input.txt")
puzzleInput
...
enum class DeckPositionState(val display: Char) {
    ...
    companion object {
        val statesByDisplay = DeckPositionState.values().associateBy { it.display }
...
fun buildGraph(layout: List<DeckPosition>): Map<DeckPosition, List<DeckPosition>> {
    val maxRow = layout.maxByOrNull { it.row }
    val maxCol = layout.maxByOrNull { it.col }
    return layout.map {
...
settleInSeats(parseInput(puzzleInput)).count { it.state == DeckPositionState.OccupiedSeat}
